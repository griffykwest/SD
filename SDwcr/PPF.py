...
def ppf(batch,z):
    ...
    heating_3d = heating_Wm.reshape((nx, ny, nz), order='F').T
    ...
    first_size=9
    full_size=17
    nz, ny, nx =  heating_3d.shape

    print(nx)

    #multiuplying for proper readings(similar to pltfix)
    weights = np.ones_like(heating_3d)
    ...
    assembly_map = np.zeros_like( heating_3d, dtype=int)
    assy_num = 0
    for i in range(len(x_edges)-1):
        for j in range(len(y_edges)-1):
            ...
            assy_num += 1
    print(assembly_map)
    datatocsv(assembly_map[:,:,z],'assy.csv')
    #datatocsv( heating_xy,'heating.csv')
    ...
    for a in range(n_assembly):
        mask = assembly_map == a
        assembly_sums[a] = np.sum( heating_3d[mask] * weights[mask])
    ...
    for a in range(n_assembly):
        ...
        mask = assembly_map == a
        pin_power =  heating_3d[mask] * weights[mask]
...
Fq_intra, Fq_inter, assembly_avg , fuel_mask = ppf(2006,1)
